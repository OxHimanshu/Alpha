...
import alphaCoreABI from '../alphaCoreABI.json';
import principalERC20ABI from '../principalERC20ABI.json';
import alphaERC20ABI from '../alphaERC20ABI.json';
...
function Yield () {
    ...
    const getBalances = async () => {
        ...
        const _alphaCoreContract = new ethers.Contract(alphaCoreContract, alphaCoreABI, provider);
    ...
    const claimRewards = async () => {
        ...
        const tokenContractAddr = new ethers.Contract(tokenContract[chain.id].ALPHA_TOKEN_CONTRACT, alphaERC20ABI, provider);
        const signedTokenContractAddr = tokenContractAddr.connect(signer)
    ...
    return (
        <div className="text-white mt-2 rounded-r-[10px] w-[1292px] h-[884px] bg-black overflow-y-scroll">
            { isConnected ? months === 0 ?
                <div>
                    <div className='text-center text-5xl mt-16 h-32'>Get Instant Yield Upfront</div>
                    <div className=' bg-orange-600 font-semibold text-center border-y border-black text-3xl py-4'>Strategies</div>
                    {
                                ...
                                <div className='grid grid-cols-4'> 
                                    {
                                        token["strategies"].map(data => {
                                            return (
                                                <div onClick={() => {setMonths(data.months); setTokenContractAdr(token.address); setToken(token["token"]); setAlphaCoreContract(data.ALPHA_CORE_CONTRACT); setLogo(token.logo)}} className='cursor-pointer flex flex-col text-lg items-center justify-center rounded-lg bg-orange-600 hover:bg-orange-500 mt-6 mx-4 w-[300px] h-60'>
                                                    <img src={token["logo"]} className="w-20 h-20 mb-4" alt=""/>
                                                    <div>Token Pool: {token["token"]}</div>
                                                    <div>Tenure: {data.months} Months</div>
                                                </div>
                                            )
                                        })
                                    }
                ...
                <div className='w-full'>
                    <div className='m-6 cursor-pointer w-14' onClick={() => setMonths(0)}> 
                        <img src={arrow} className="rounded-full p-3 w-12 h-12 bg-orange-600"/>
                    </div>
                    <div className='w-full flex flex-row justify-between'>
                        <div className='flex items-center justify-center h-[750px] w-[640px]'>
                            <div className='flex flex-col items-center space-y-1'>
                                    ...
                                    <div className='border-t-2 border-orange-600 p-2'> 
                                        <div className='flex justify-center space-x-16'>
                                            <div className='text-xl py-2'> Rewards: {totalRewards} </div>
                                            <button onClick={() => claimRewards()} className='rounded-lg bg-orange-600 font-semibold px-3'>CLAIM</button>
                                        </div>
                                    ...
                                    <div className='pt-4 border-orange-600 border-t-2 p-4'>
                                        <div className='flex flex-col text-md text-end items-end'>
                                            <div className='flex my-1'>
                                                ...
                                                <button onClick={() => stakeAmount()} className='rounded-lg bg-orange-600 text-white w-24 ml-4 text-lg font-semibold'>Stake</button>
                                            </div>
                                            ...
                                            <div className='flex my-1 mt-4'>
                                                ...
                                                <button onClick={() => unstakeAmount()} className='rounded-lg bg-orange-600 text-white w-24 ml-4 text-lg font-semibold'>Unstake</button>
                                            </div>
                                    ...
                                    <div className='pt-4 border-orange-600 border-t-2 p-4'>
                                        <div className='flex flex-col text-md text-end items-end'>
                                            <div className='flex my-1 mt-4'>
                                                ...
                                                <button onClick={() => mintPE()} className='rounded-lg bg-orange-600 text-white w-32 ml-4 text-lg font-semibold'>Mint Energy</button>
                                            </div>
                                        ...
                                        <div className='flex flex-col text-md text-end items-end'>
                                            <div className='flex my-1 mt-4'>
                                                ...
                                                <button onClick={() => burnPE()} className='rounded-lg bg-orange-600 text-white w-32 ml-4 text-lg font-semibold'>Burn Energy</button>
                                            </div>
