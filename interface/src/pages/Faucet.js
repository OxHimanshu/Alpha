...
function Faucet () {
    ...
    return (
        <div className="text-white mt-2 rounded-r-[10px] w-[1292px] h-[884px] bg-black">
            {
                isConnected ?
                    <div>
                            ...
                            <div className='space-x-24'>
                                {
                                    activeStrategies[chain.id].map(token => {
                                        return (
                                            <button onClick={() => mintToken(token.address)} className='rounded-[10px] w-[160px] h-[80px] bg-orange-600 text-xl font-semibold'>Mint {token.token}</button>
                                        )
                                    })
                                }
                            </div>
